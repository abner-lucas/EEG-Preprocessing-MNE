...
for sub in subjects:
    subject_id = str(sub[-4:])
    format_log = '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
    mne.set_log_file('/content/drive/MyDrive/Colab Notebooks/PPGCC/EEG/outputs_eeg/subject' + subject_id + '_log.log', output_format=format_log, overwrite=True)
    ...
    if stimulus != None:
        ...
        if subject_id in list_control:
            group_subject = 'control'
        elif subject_id in list_gifted:
        ...
        df_ip.to_csv('/content/drive/MyDrive/Colab Notebooks/PPGCC/EEG/outputs_eeg/subject' + subject_id + '_ip.csv',  index = False)
        #df_ip.to_csv('outputs_eeg/subject' + subject_id + '_ip.csv', index = False)

        df_eeg, df_evoked = pproc.get_data_frame()
        df_eeg = df_eeg.rename(columns={"epoch":"trial"})
        df_eeg.insert(0, 'subject_id', subject_id)
        df_eeg.insert(1, 'group', group_subject)
        df_evoked.insert(0, 'subject_id', subject_id)
        df_evoked.insert(1, 'group', group_subject)

        df_eeg.to_csv('/content/drive/MyDrive/Colab Notebooks/PPGCC/EEG/outputs_eeg/subject' + subject_id + '_trials.csv',  index = False)
        df_evoked.to_csv('/content/drive/MyDrive/Colab Notebooks/PPGCC/EEG/outputs_eeg/subject' + subject_id + '_evoked.csv', index = False)
        #df_eeg.to_csv('outputs_eeg/subject' + subject_id + '_trials.csv', index = False)
        #df_evoked.to_csv('outputs_eeg/subject' + subject_id + '_evoked.csv', index = False)